...
class RegForm(forms.Form):
    username = forms.CharField(label='用户名',
                                max_length=30,
                                min_length=3,
                                widget=forms.TextInput(attrs={'class':'form-control', 'placeholder':'请输入不少于3个字符的用户名'}))
...
class ChangeNicknameForm(forms.Form):
    ...
    def clean(self):
        ...
        if self.user.is_authenticated: 
            ...
            raise forms.ValidationError('用户未登录，请重新登录后再试~')
        return self.cleaned_data
...
class BindEmailForm(forms.Form):
    ...
    def clean(self):
        ...
        '''
        # 判断用户是否已绑定邮箱 [注释掉这一块，让用户能够解绑其他邮箱]
